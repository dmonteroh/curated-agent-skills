# Database Cost Optimization Playbook

This playbook focuses on database-specific cost drivers and safe levers. It is intentionally tooling-agnostic.

## Step 1: Establish a baseline

Capture:
- cost over the last 30/60/90 days (or best available window)
- DB resource metrics: CPU, memory, IO/IOPS, storage growth, connections
- top queries by total time and by mean latency (if available)
- reliability posture: RPO/RTO, HA requirements, read replica needs

## Step 2: Identify primary cost drivers

Typical drivers:
- **Compute class**: overprovisioned CPU/memory
- **IO**: chatty queries, missing indexes, vacuum/autovacuum issues, write amplification
- **Storage**: retention, bloat, backups/snapshots, logs, large indexes
- **Replicas/HA**: more replicas than needed, oversized standby nodes

## Step 3: Safe levers (usually low risk)

- Clean up retention policies (old snapshots/backups/logs) *with explicit recovery requirements*.
- Fix expensive queries and missing indexes (often reduces IO and compute).
- Reduce idle connections and right-size pool settings (prevents forced bigger instances).
- Enable compression/tiering where supported and operationally safe.

## Step 4: Medium-risk levers (gate with metrics)

- Downsize instance class (only after proving utilization headroom).
- Reduce replica count (only if read load and failover requirements still hold).
- Change storage class or provisioned IO (ensure you won’t hit latency cliffs).

## Step 5: High-risk levers (plan like a migration)

- Engine changes, major version upgrades, topology changes.
- Sharding/partitioning purely for cost.

Treat these as a track with:
- explicit rollback plan
- canary/staging validation
- objective go/no-go gates (latency, errors, replication lag)

## Output template (what to deliver)
