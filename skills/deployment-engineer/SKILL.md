...
- Verifying an artifact that a release step rewrites on the way to production (repackaging, minification, re-encoding, quantization), where a successful load is not evidence that it still behaves
...
- The question is which format, codec, or compression an artifact should be transformed into, rather than how the release proves the transformed artifact still works
