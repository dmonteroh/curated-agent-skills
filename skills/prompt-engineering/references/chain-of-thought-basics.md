...
> **Reasoning models**: models with built-in extended/adaptive thinking reason before answering on their own — manual CoT cues like "think step by step" are redundant there and can reduce quality. Control reasoning depth via the API's thinking/effort settings instead, and reserve the patterns below for non-reasoning models or for cases where the visible response itself must contain auditable steps. See `frontier-model-prompting.md`.
