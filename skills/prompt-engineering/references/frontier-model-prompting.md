...
5. **Audit inherited hedges before writing new ones.** The instinct on a migration is to add instructions for the new model; do the subtraction pass first. A hedge exists because some *earlier* model over-complied loosely — it rambled, generalized an instruction to items it was never given, or volunteered work nobody asked for — and its wording was tuned against that looseness. A model that follows instructions more literally executes the wording as written, so the same hedge now lands on cases it was never meant to cover, or applies with a rigidity the author never intended. Walk the existing directives, ask what failure each was written against, and re-baseline the ones whose target model is gone. This is triggered by a symptom — the new model reads narrower, terser, or more literal than expected — not by any particular model name or version.
6. **Check effort parity before blaming the prompt.** If output looks shallow on a genuinely hard task after the switch, confirm the new model was given a comparable reasoning budget before concluding the prompt text is defective. A migration moves the effort scale as well as the model: default levels differ, and a budget that was generous on the old model can be modest on the new one. Rule out the setting first, then edit wording.
