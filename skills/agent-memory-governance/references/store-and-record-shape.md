...
## Error policy on the write side

- A **throttle** signal means defer the save and move on. The content is not lost, only not persisted this run.
- Any other non-zero result is transient in the same way: proceed, do not retry inline, do not fail the user's task.

Neither case is an error the user has to act on, and neither is a reason to stop work.
