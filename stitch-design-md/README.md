...
```bash
npx add-skill google-labs-code/stitch-skills --skill stitch-design-md --global
```
...
1. **Retrieval (preferred)**: Uses the Stitch MCP Server to fetch project screens, HTML code, and design metadata.
   - If the MCP server is not available, provide the HTML/CSS + screenshots manually.
