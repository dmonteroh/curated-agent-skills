...
- Preferred: access to the Stitch MCP Server (to fetch screen metadata + HTML automatically).
- Alternative: if the MCP server is not available, you can still do the job if the user provides:
  - exported HTML/CSS (or a repo path to it)
  - one or more screenshots of the target screen(s)
  - project title (and optionally IDs)
...
To analyze a Stitch project, prefer retrieving screen metadata and design assets using the Stitch MCP Server tools.

If the Stitch MCP server is not available, skip these steps and instead ask the user for:
- the HTML/CSS source (file path or pasted snippet)
- screenshots or exported images
- any existing theme/tokens files

1. **Namespace discovery (optional)**: Run `list_tools` to find the Stitch MCP prefix. Use this prefix (e.g., `mcp_stitch:`) for all subsequent calls.
