...
- Investigating slow queries, timeouts, lock contention, or saturation.
- Designing indexes/queries based on observed access patterns.
- Tuning connection pooling, caching, and partitioning for performance.

## Do not use this skill when

- You’re primarily doing data modeling/technology selection (use a database architecture skill).
- The bottleneck is clearly outside the database (CPU in app, network, etc.).

## Instructions

- Capture symptoms and constraints:
  - p95/p99 latency, throughput, error rates
  - top slow queries/endpoints
  - concurrency level, timeouts, pool sizes
  - DB engine/version and key settings
- Identify the bottleneck class:
  - query plan/indexing
  - locks/contention
  - connection pool saturation
  - IO/memory pressure
  - write amplification / bloat
- Propose fixes with verification:
  - EXPLAIN/plan deltas, index changes, query rewrites
  - migration-safe rollout (concurrent index builds where supported)
  - post-change validation and rollback plan

If you need detailed checklists and common fixes, open:
